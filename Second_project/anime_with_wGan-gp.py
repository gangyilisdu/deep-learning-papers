...
for epoch in range(num_epochs):
    for i, data in enumerate(dataloader, 0):
        ...
        print(real_cpu.size(), output.data.size())
        X_HAT = alpha * real_cpu + (1 - alpha) * output.data
        print(X_HAT.size())
        ...
        gradient_penalty = 10 * ((gradients.view(gradients.size()[0], -1).norm(2, 1) - 1) ** 2).mean()  # 10 is the lambda
