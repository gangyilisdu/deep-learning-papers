...
def collect_and_transfer(input):
    ...
    output = Variable(torch.tensor([item for item in output]))
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(1, 1, kernel_size= (4, 1), stride= (4, 1))
        self.conv2 = nn.Conv2d(1, 1, kernel_size= (4, 1), stride= (4, 1))
        self.conv3 = nn.Conv2d(1, 1, kernel_size= (1, 2), stride= (1, 1))
        self.conv4 = nn.Conv2d(1, 1, kernel_size= (1, 2), stride= (1, 1))
        self.bn1 = nn.BatchNorm2d(1)
        self.bn2 = nn.BatchNorm2d(1)
        self.relu = nn.ReLU()
        self.sigmoid = nn.Sigmoid()
        self.Linear1 = nn.Linear(334, 168)
    def forward(self, input):
        output = collect_and_transfer(input)
        ...
        final_output = torch.cat([output1, output2], 3)
        #print(final_output.size())

        final_output = self.Linear1(final_output)
        #final_output = self.sigmoid(final_output)
        #print(self.conv1.weight.data)
        #print(self.conv2.weight.data)
        #print(self.conv3.weight.data)
...
for epoch in range(num_epochs):
    for i, data in enumerate(D_dataLoader):
        ...
        label = torch.full((batch_size,), 0, dtype=torch.float)
        ...
        noise = torch.zeros(45, 168, dtype=torch.float64)
        noise = noise + (0.1 ** 0.5) * torch.randn(45, 168)
        noise += data[1]
...
print(pred)
np.savetxt('res1.csv', pred, delimiter=',')
